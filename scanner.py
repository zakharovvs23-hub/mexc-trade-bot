...
# Список монет по умолчанию для /scan без аргументов (топ-50)
TOP_COINS = [
...
# Фиксированные пулы ротации — весь охват тикеров, который раньше собирался
# и сканировался вручную через Claude. /pool a ... /pool e сканирует их
# напрямую через MEXC, без прокси и без ручной пересылки списков.
POOLS = {
    "a": [
...
def scan_coins(coins: list[str]) -> tuple[list[dict], list[tuple[str, str]]]:
    """
    Прогоняет analyze_raw (полный Elder's Triple Screen) по списку монет
    и сортирует результаты по score (BUY сильнее WATCH, WATCH сильнее WAIT).
...
def format_scan_result(results: list[dict], errors: list[tuple], title: str | None = None) -> str:
    """
    Пулом выводит по каждой монете всю ту же информацию, которую Вадим
    проверяет вручную: сигнал, цену, недельный тренд (Screen 1), Elder-ray
    Bull/Bear Power (Screen 2), RSI как вторичное подтверждение, score.
    """
    ...
    lines = [
        f"📊 {header}, от сильных к слабым: 🟢 BUY {buy_count} | 🟡 WATCH {watch_count}",
        "",
    ]
    ...
    if watch_count:
        lines.append("")
        lines.append("🟡 WATCH — недельная структура уже бычья (Screen 1 пройден), но точка входа "
